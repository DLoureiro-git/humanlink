...
import { Loader2, Ban, Trash2, Plus, Search, ChevronLeft, ChevronRight } from 'lucide-react';
  ...
  const filtered = useMemo(() => {
    ...
    return users.filter(
      (u) =>
        ...
        (u.personality_choice && u.personality_choice.toLowerCase().includes(q))
    );
  }, [users, search]);
    ...
    <div>
      ...
      <div className="mb-4 relative">
        ...
        <input
          ...
          className="w-full rounded-xl border border-white/10 bg-white/[0.04] py-2.5 pl-10 pr-4 font-body text-sm text-white placeholder-white/30 outline-none transition-colors focus:border-accent-purple/50"
        />
      </div>
        ...
        {totalPages > 1 && (
          <div className="flex items-center justify-between border-t border-white/[0.06] px-6 py-3">
            <p className="font-body text-xs text-white/30">
              {filtered.length} {t('total_users').toLowerCase()}
            </p>
            <div className="flex items-center gap-2">
              <button
                onClick={() => setPage((p) => Math.max(0, p - 1))}
                disabled={page === 0}
                className="rounded-lg border border-white/10 p-1.5 text-white/40 transition-colors hover:bg-white/[0.06] hover:text-white disabled:opacity-30"
              >
